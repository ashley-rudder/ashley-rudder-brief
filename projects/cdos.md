...
## Poppy front: 9/23 call with Mo, deadline day

Mo (Poppy build lead) called Ashley midday, eleven minutes late, and laid out the build scope: payments, login, paywall, auto-downgrade on cancel, onboarding fix, all bots, post DNA, IP vault, governance dashboard, content engine with post scoring and rewrite, calendar events auto-updating on sprint changes, landing page fixed for desktop and mobile. Excluded with reasons given: LinkedIn feed scan and auto-posting (API partner approval, a real constraint), full social tracking, Canva API, full two-way Google Calendar API sync (event updates stay), and live voice replies (voice DNA stays, the bot speaking back does not). Store approvals explicitly carved out as Ashley's own track, which she confirmed. One pricing strategy call and one round of revisions offered. Poppy boards content setup, part of the original sale conversations, confirmed excluded; flagged as the value gap to reinstate, price, or trade in writing.

Mo committed to sending an official agreement the same day, to be run past Cameron and Qazi. The disclosure that matters: Mo has not spoken to Qazi about the IP assignment and does not think it will be a problem. The assignment deadline Ashley set is Wednesday end of day, which is today, and the draft is not yet signable because Poppy's registered legal name is still outstanding (requested from Qazi by email). Ruling path drafted for Ashley's reply: get the entity details from Mo directly today so the document goes final this afternoon and the deadline holds; review Mo's build agreement now but sign only alongside or after the executed IP assignment; attorney reads both so the IP language cannot conflict; push the agreement to include timeline, milestones, payment terms, acceptance criteria, revisions per milestone, communication cadence, and Lovable access in writing. Qazi's signature stays the unlock.

Afternoon of the same day, the paper trail assembled. Ashley produced the July record: the CDOS V2 plan site (cdos-v2-plan.vercel.app, "Ashley Rudder x Athena") and the unsigned July 31 Delivery Agreement packet that followed the July 31 call, quoting $6,500 flat for four weeks covering both halves: the build (stabilization, Stripe off Stan, Head Down built out with social tracking, onboarding pass, trim, launch readiness, education session) and the content system (naming and positioning, two Poppy boards for LinkedIn and shortform, three working sessions with Charan Singh of Poppy's content coaching). That quote is what flabbergasted her, because the content system had been represented as part of her package before it wore a price. Never signed, never paid; it is ammunition, not obligation. Mo's Option 1 today is broader on build (all bots, governance dashboard, content engine, review fixes) but strips the entire content half and social tracking.

Transcripts landed late afternoon: the Chelsea call and the Cameron plus Mo call, both from the day before. Hard facts now on the record. The original purchase was $10,000 for the full ledger (twenty-one items, listed at Ashley's own February onboarding, in the project hub and the shared Slack). Cameron twice and Mo once put a full refund on the table as the alternative, which makes the standing BATNA: walk with the $10,000 back, keep the knowledge base, self-host the app she has substantially built herself. Mo's task from that call was to review the recordings and the ledger and return with a scope he could commit to; Option 1 is that answer and Option 2 was always the refund. Overnight movement confirms the tapes worked: the day before, Mo disputed the bots and the content engine as phase-two extras; Option 1 includes all four bots, the content engine, and the governance dashboard. Option 1 showed no price; the position is that it executes against the $10,000 already paid, zero new fees, and any number on Mo's draft gets answered with the refund they offered. Athena is no longer with Poppy, which reassigns the build education session to Mo and supplies the argument for written scope: the dispute happened because scope lived in one head and that head left. Chelsea flagged exactly three fixes to the IP assignment, called it otherwise good enough to negotiate on, cautioned that Lovable and third-party tool terms under the build affect ownership of generated work, and confirmed the call recordings are safe leverage from Texas against New York. Cameron and Mo said on the record they have no problem signing the assignment; their questions were wording clarifications on sandboxing and removal language. Chelsea also ruled, separately: Creator Darwinism should be its own entity with its own EIN, walled from HAUS OF SOS, possibly under an IP holding structure with Always Be Saucy LLC licensing down; the software trademark filing needs the entity first; classes 9 and 42 look clear, class 35 carries a Content Darwinism yellow flag; Always Be Saucy is registered, HAUS OF SOS statement of use processed September 2 with registration expected within two months; James Creedon referral queued for the method patent and AI terms.

Record correction from Ashley, same evening: in the earlier Athena conversations, the content strategist was originally Christian; Athena said Christian was not available and brought in Charan instead. The strategist commitment therefore predates and survives any individual: Poppy promised the role and has already swapped the person once for availability. Her call recordings hold that exchange. Ruling applied: the Qazi email and the agreement language name the role, not the person ("a senior content strategist designated by Poppy, currently Charan Singh"), so roster churn can never erase the obligation the way Athena's exit almost erased the scope.

Sharpened the same evening, then verified against Gmail primary sources. Corrections to the two lines above: Charan is Poppy's own (charan@getpoppy.ai, "of Poppy's content coaching team" per the July agreement), not an outside vendor; and the total invested is $16K on the record, not $10K (the $10K was the build alone). The receipts, dated: Ashley's 7/26 email to Athena, CC christian@getpoppy.ai, "Looking forward to catching up early next week with you and Christian"; the Grain summary of the 7/31 call, in the inbox since that afternoon, stating Charan Singh joined "replacing the unavailable Christian"; and Camryn Ward on that same 7/31 record saying Charan's content work, not the build, "accounts for the majority of the $6,500 charge." So Option 1 today excludes what Cam himself called the majority of the value. Also on the 7/31 record: Athena updated contract language to state features built for Ashley are proprietary to Ashley alone, Poppy will not reuse them; the IP position Qazi is now signing was Poppy's own position in July. Grain recording: grain.com/share/recording/22dd7a4d-74a6-42ac-83a9-ab8cdf95c522.

Standing note: Ashley assembled the canonical receipts dossier herself on 9/19 and sent it to Qazi and the team ("Creator Darwinism OS: what was specified, what exists, and what I need signed," every row citing a Poppy document or message), with the draft IP assignment following the same day. The desks reference that dossier and this charter before re-deriving anything.

Negotiating position set, email to Qazi drafted and finalized: entity details today, deadline holds, IP assignment signs on its own track; the new agreement carries the July content system included at no cost per their own representation, a strategist through launch, the free Power User year with full credits commencing at launch (she bought launch capacity that never became usable), dated milestones on Athena's own four-week skeleton with a remedy ladder (pay in arrears per milestone, short cure, weekly credits, termination with reach-back refund, IP and access surviving), and a kickoff re-baseline against the September app so nobody pays four weeks for work Sable already shipped. Any new price at or above $6,500 with the content half removed is worse than the July offer by their own page. The packet, plan screenshots, and Cam's text belong in the Drive admissions record.

## Session log: 9/23 payments desk, seventeenth pass. Rejected at dawn, selling through Apple by night.

Five items submitted to App Review at 8:21 PM CDT the same day the 3.1.1 rejection landed at 7:30 AM: iOS App 1.0 build 14, the three auto-renewable subscriptions (Monthly $99.99, Power User $299.99, Ultra $499.99), and the CDOS Membership subscription group. Apple quotes up to 48 hours, verdict by email.

The full In-App Purchase rail went from nonexistent to revenue-verified in one day. Code: RevenueCat via purchases-capacitor 13.6.0, native paywall at /upgrade in the house editorial system, every native upgrade surface routed to it, iap-sync edge function provisioning from RevenueCat as source of truth on both the instant client path and the webhook path (commits fc99357 through 862fee3; the Lovable agent contributed a type-narrowing fix and two error-handler hardenings, audited and adopted). Config: Paid Apps Agreement signed and Active same day with banking and W-9, three subscriptions built in App Store Connect with the paywall capture as review screenshot, RevenueCat credentialed with the In-App Purchase key ("Valid credentials"), products mapped, default offering stocked, secrets seated in the Lovable Cloud vault, webhook registered.

Proof before submission, not after: a fresh free-tier tester (paywalltest@hausofsos.com) walked the new-customer path on build 14, hit See Plans on the linking screen (the 3.1.1 pothole paved in 862fee3 after build 12 exposed it as a dead end), rendered live Apple prices, and completed a sandbox purchase of Monthly. The database receipt landed the same minute: paid_users active with product Apple Subscription (monthly), user_credits flipped to unlimited sessions. Apple to RevenueCat to webhook to Supabase, end to end, zero dollars.

Builds 12 through 14 in one evening (12 pre-See-Plans, 13 archived before the pull, 14 the real one; lesson standing: read the archive's version line in the Organizer before uploading). The submission maze, recorded for the next fight: first-time subscriptions ride only inside a draft submission with the app version AND the subscription group item; the version page's Add for Review disappears once staged; the rejected submission held the version hostage until Cancel Submission released it; the group has its own Add for Review that clears the final warning.
